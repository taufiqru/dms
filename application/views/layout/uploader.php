       ...
       var manualUploader = new qq.FineUploader({
            ...
            callbacks:{
              onError : function(id, name, errorReason, xhrOrXdr) {
                        var msg = qq.format("Error on file number {} - {}.  Reason: {}", id, name, errorReason);
                        var obj = document.createElement('span');
                        obj.setAttribute('class','qq-upload-status-text-selector qq-upload-status-text');
                        var txt = document.createTextNode(msg); 
                        obj.appendChild(txt); 
                        document.getElementsByClassName("qq-file-id-"+id)[0].appendChild(obj);
                        },
              onComplete : function(id,name,response,xhr){
                              console.log(response);
                              if(response.success){
                                var url = '<?=base_url()?>index.php/dokumen/recordUploadedDoc/';
                                $.post(url,{folder:'<?=$_GET["id"]?>',filename:response.filename}).done(function(){});
                              }
                           }
            },
            autoUpload: true,
